...
void reset()
{
   ...
   GLfloat x=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))*0.1;
    GLfloat y=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))*0.1;
     GLfloat z=1+(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))*0.1;
    GLfloat Ax=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))-0.5;
    GLfloat Ay=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))-0.5;
    GLfloat Az=3+(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)));
...
void createParticules(int nbParticules )
{
  ...
  {
    GLfloat x=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))*0.01;
    GLfloat y=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))*0.01;
     GLfloat z=1+(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))*0.01;
    GLfloat Ax=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))-0.5;
    GLfloat Ay=(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)))-0.5;
    GLfloat Az=3+(static_cast <double> (rand()) / (static_cast <double> (RAND_MAX)));
    listeParticules.push_back(Particule{{x,y,z},{Ax,Ay,Az} ,0.01  });
  }
...
void computeX(Particule & p ,float delta )
{
  ...
  float vz=p.vitesse[2];
   ...
   p.vitesse[0]=vx+(g.x)*(delta);
   p.vitesse[1]=vy+(g.y)*(delta);
   p.vitesse[2]=vz+(g.z)*((double)delta);
...
{
for(int j=N*(i%nbgenerer);j<N*(i%nbgenerer)+N;j++)
listeParticules[j].reset();

//cout<<j<<endl;
}
  for(int J=0;J<listeParticules.size();J++)
    computeX(listeParticules[J], (double)delatTemps/1000);
